...
import { useUpdateZone } from "@/store/server/zone/mutation";
import { useGetZoneById } from "@/store/server/zone/queries";
...
const UpdateZone = () => {
  ...
  const { data: zoneData } = useGetZoneById(id?.toString() || "");

  const updateZone = useUpdateZone(id?.toString() || "");

  useEffect(() => {
    if (zoneData?.data) {
      form.setValue("countryId", zoneData.data.country.id);
      form.setValue("name", zoneData.data.name);
      form.setValue(
        "cities",
        zoneData.data.cities.length > 0
          ? zoneData.data.cities
          : [{ id: "", townshipList: [] }]
      );
    }
  }, [zoneData]);
  ...
  return (
    <div className=" px-5 pt-5">
      ...
      <div className=" mt-5 p-5 bg-white">
        <Form {...form}>
          <form
            onSubmit={form.handleSubmit((val) => updateZone.mutate(val))}
            id="form-data"
            name="form-data"
            className=" space-y-5"
            action=""
